...
export function AsanaWidget({ context }: PluginWidgetProps) {
  const { data: stats, loading } = usePluginData<AsanaStats>("stats", {});
  ...
  return (
    <div style={card}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "12px" }}>
        <span style={{ fontSize: "14px", fontWeight: 600 }}>Asana MCP</span>
        {stats.mcpRunning
...
export function AsanaPage({ context }: PluginPageProps) {
  const { data: stats, refresh } = usePluginData<AsanaStats>("stats", {});
  const { data: config } = usePluginData<AsanaConfigView>("config", {});
...
export function AsanaSettings({ context }: PluginSettingsPageProps) {
  const { data: config } = usePluginData<AsanaConfigView>("config", {});
  const testConnection = usePluginAction("test-connection");
