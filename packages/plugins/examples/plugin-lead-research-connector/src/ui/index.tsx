...
interface LeadResearchConfigView {
  hasToken: boolean;
  mcpUrl: string;
}
...
export function LeadResearchWidget({ context }: PluginWidgetProps) {
  const { data: stats, loading } = usePluginData<LeadResearchStats>("stats", {});
  ...
  return (
    <div style={card}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "12px" }}>
        <span style={{ fontSize: "14px", fontWeight: 600 }}>Lead Research MCP</span>
        {stats.mcpRunning
...
export function LeadResearchPage({ context }: PluginPageProps) {
  const { data: stats, refresh } = usePluginData<LeadResearchStats>("stats", {});
  const { data: config } = usePluginData<LeadResearchConfigView>("config", {});
  ...
  return (
    <div style={{ maxWidth: "720px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "20px" }}>
      <div>
        <h2 style={{ fontSize: "20px", fontWeight: 700, color: "var(--foreground)", margin: 0 }}>Lead Research Connector</h2>
        <p style={{ fontSize: "13px", color: "var(--muted-foreground)", margin: "4px 0 0" }}>
          Thin MCP bridge — connects to your remote Lead Research MCP server and proxies its tools to Paperclip agents.
        </p>
      </div>
      ...
      <div style={card}>
        ...
        <ol style={{ margin: 0, paddingLeft: "20px", fontSize: "13px", color: "var(--muted-foreground)", lineHeight: 1.8 }}>
          <li>Configure your remote MCP URL and auth token in Settings.</li>
...
export function LeadResearchSettings({ context }: PluginSettingsPageProps) {
  const { data: config } = usePluginData<LeadResearchConfigView>("config", {});
  const testConnection = usePluginAction("test-connection");
  ...
  return (
    <div style={{ maxWidth: "560px", display: "flex", flexDirection: "column", gap: "20px" }}>
      <div>
        <h3 style={{ fontSize: "16px", fontWeight: 600, margin: 0 }}>Lead Research Settings</h3>
