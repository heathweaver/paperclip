...
class McpHttpClient {
  ...
  async start(): Promise<void> {
    ...
    await this.send("initialize", {
      ...
      capabilities: {},
      clientInfo: { name: "paperclip-lead-research-connector", version: "0.1.0" },
    });
...
const PLUGIN_NAME = "lead-research-connector";
      ...
      for (const tool of discoveredTools) {
        ctx.tools.register(
          tool.name,
          {
            ...
            description: tool.description ?? `Lead Research MCP tool: ${tool.name}`,
