...
export function FreshsalesWidget({ context }: PluginWidgetProps) {
  ...
  return (
    <div style={card}>
      ...
      {stats.bundleAlias && <div style={{ fontSize: "12px", color: "var(--muted-foreground)", marginBottom: "8px" }}>{stats.bundleAlias}.myfreshworks.com</div>}
      <div style={statBox}>
    ...
    <div style={{ maxWidth: "720px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "20px" }}>
      <div>
        ...
        <p style={{ fontSize: "13px", color: "var(--muted-foreground)", margin: "4px 0 0" }}>
          Thin MCP bridge — spawns your Freshsales MCP server and proxies its tools to Paperclip agents.
        </p>
      </div>
      ...
      <div style={card}>
        ...
        {stats?.bundleAlias && <div style={{ fontSize: "12px", color: "var(--muted-foreground)", marginBottom: "12px" }}>Instance: {stats.bundleAlias}.myfreshworks.com</div>}
      ...
      {stats && stats.toolCount > 0 && (
        <div style={card}>
          <div style={{ fontSize: "14px", fontWeight: 600, marginBottom: "8px" }}>
            Discovered Tools ({stats.toolCount})
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
            {stats.toolNames.map((name) => (
              <span key={name} style={mono}>{name}</span>
            ))}
          </div>
        </div>
      )}
      ...
      <div style={card}>
        ...
        <ol style={{ margin: 0, paddingLeft: "20px", fontSize: "13px", color: "var(--muted-foreground)", lineHeight: 1.8 }}>
          <li>Configure your Freshsales API key, bundle alias, and MCP server command in Settings.</li>
      ...
      {config && (
        <div style={card}>
          ...
          <table style={{ width: "100%", fontSize: "13px", borderCollapse: "collapse" }}>
            <tbody>
              <tr><td style={{ padding: "4px 8px 4px 0", color: "var(--muted-foreground)" }}>API Key</td><td style={{ fontWeight: 500 }}>{config.hasApiKey ? "Configured" : "Not set"}</td></tr>
              <tr><td style={{ padding: "4px 8px 4px 0", color: "var(--muted-foreground)" }}>Bundle Alias</td><td style={{ fontWeight: 500 }}>{config.bundleAlias || "—"}</td></tr>
    ...
    <div style={{ maxWidth: "560px", display: "flex", flexDirection: "column", gap: "20px" }}>
      <div>
        ...
        <p style={{ fontSize: "12px", color: "var(--muted-foreground)", margin: "4px 0 0" }}>
          Configure API key, bundle alias, and MCP server command via the plugin instance config.
        </p>
      </div>
      <div style={card}>
        ...
        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
          ...
          {config?.hasApiKey ? <span style={badgeStyle("oklch(0.72 0.17 142)")}>Key set</span> : <span style={badgeStyle("oklch(0.80 0.15 85)")}>No key</span>}
        </div>
        {config && config.mcpArgs && <div style={{ marginTop: "8px", fontSize: "12px", color: "var(--muted-foreground)" }}>Server: <span style={mono}>{config.mcpCommand} {config.mcpArgs}</span></div>}
